...
class ManipulationManager:
  ...
  def __init__(self, NET_TRANS_NODE, SCENEGRAPH, SCENE_MANAGER):
   ...
   # self.ray_pointer1.my_constructor(self, 1, SCENEGRAPH, NET_TRANS_NODE, _parent_node, _transmitter_offset, "tracking-dlp-pointer1", "device-pointer1")
    ...
    pointer_tracking_sensor.Station.value = "tracking-dlp-pointer1"
    pointer_tracking_sensor.ReceiverOffset.value = avango.gua.make_identity_mat()
    pointer_tracking_sensor.TransmitterOffset.value = _transmitter_offset

    _mouse_input = MouseRayInput(_parent_node,  'device-mouse')
    _sf_mat = _mouse_input.pickray_matrix

    _mouse_input2 = MouseRayInput(_parent_node,  'device-mouse2')
    _sf_mat2 = _mouse_input2.pickray_matrix
    #_sf_mat = pointer_tracking_sensor.Matrix

    _distance_selector = DistanceSelector()
    _distance_selector.my_constructor(NET_TRANS_NODE)

    _ray = Ray()
    _ray.my_constructor(1, SCENEGRAPH, NET_TRANS_NODE, _parent_node, _sf_mat, 10, 0.5)

    _ray2 = Ray()
    _ray2.my_constructor(2, SCENEGRAPH, NET_TRANS_NODE, _parent_node, _sf_mat2, 10, 0.5)

    
    self.ray1 = myInteractionTool()
    self.ray1.my_constructor(_ray, _sf_mat, 'device-mouse', _distance_selector)

    self.ray2 = myInteractionTool()
    self.ray2.my_constructor(_ray2, _sf_mat2, 'device-mouse2', _distance_selector)
