...
class PortalInteractionSpace(avango.script.Script):
  ...
  def my_constructor(self, DEVICE, PLATFORM, MIN_POINT, MAX_POINT, FORWARD_ANGLE):
    ...
    ## @var animation_forward_angle
    # Angle in degrees in which a portal will face at the end of the animation.
    self.animation_forward_angle = FORWARD_ANGLE
  ...
  def evaluate(self):
    ...
    _plane_transform = avango.gua.make_trans_mat(_avg_point.x, self.MIN_POINT.y, _avg_point.z) * \
                       avango.gua.make_rot_mat(self.animation_forward_angle, 0, 1, 0) * \
    ...
    if self.maximized_portal != None:
      _forward_yaw = avango.gua.make_rot_mat(self.animation_forward_angle - self.forward_angle, 0, 1, 0)
