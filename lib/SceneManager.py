...
class DayAnimationUpdate(avango.script.Script):
  ...
  def update(self):
    ...
    if _sun_angle < 45:   # between morning and noon
      self.sf_sun_color.value = self.lerp_color(self.morning_sun_color, self.noon_sun_color, _sun_angle / 45.0)
    elif (_sun_angle > 135) and (_sun_angle < 180):   # between noon and evening
...
class SceneManager:
  ...
  def __init__(self, LOADER, PARENT_NODE):
    ...
    PARENT_NODE.Children.value.append(self.town)
    ...
    PARENT_NODE.Children.value.append(self.water)
    self.water_updater = TimedMaterialUniformUpdate()
    self.water_updater.MaterialName.value = "Water"
    ...
    PARENT_NODE.Children.value.append(self.ship_transform)
    ...
    PARENT_NODE.Children.value.append(self.plank_transform)
    '''
    ...
    PARENT_NODE.Children.value.append(self.sun)   
